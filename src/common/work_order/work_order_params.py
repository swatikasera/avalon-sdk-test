...
from utility.hex_utils import byte_array_to_hex
...
class WorkOrderParams():
    ...
    """
        Helper Function to create a work order request params.

        Parameters:
        work_order_id                        Work order ID
        worker_id               Worker ID value derived from the worker's DID
        workload_id             id of the workload to be executed by the worker. Optional if the worker
                                includes a single workload
        requester_id            Requester ID
        session_key       
        session_iv
        requester_nonce         random string generated by the participant. Used to calculate a hash of
                                this work order request
        verifying_key           Optional
        payload_format          "JSON-RPC"
        response_timeout_msecs  timeout in msecs that the caller will wait for response
        result_uri              Optional uri where the word order result will be submitted
        notify_uri              Optional uri which is notified post Work order completion
        worker_encryption_key   Optional worker encryption key used for this Work Order
        data_encryption_algorithm Optional algorithm for encrypting the data in this work order
        encrypted_session_key   Optional one-time encryption key generated by participant submitting the
                                work order
        """
    ...
    def create_request(
        ...
        JsonValidator.json_validation(0, "sdk_WorkOrderSubmit", self.params_obj)
        ...
        if encrypted_session_key is None:
            try:
                ...
                logging.info("Error while setting encrypted session key")
                logging.info(dir(err))
                message = err
                raise InvalidParamException(message, 0)
    ...
    def add_in_data(self, data, data_hash=None,
        ...
        if data is None:
            message = "Invalid data format for in data"
            raise InvalidParamException(message,0)
    ...
    def add_out_data(self, data, data_hash=None,
        ...
        if data is None:
            message = "Invalid data format for out data"
            raise InvalidParamException(message, 0)
    ...
    def validate_params(self):
        json_validation(0, "sdk_WorkOrderSubmit", self.params_obj)
