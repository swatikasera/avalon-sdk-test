...
from enums.error_code import SignatureStatus

logging.basicConfig(
    format="%(asctime)s - %(levelname)s - %(message)s", level=logging.INFO)

# Base Generic Client for TEE Worker
class BaseGenericClient(GenericClientInterface):
    ...
    def do_worker_verification(self, worker_obj):
        ...
        """
        encryption_key_signature = worker_obj['workerTypeData']['encryptionKeySignature']
        if encryption_key_signature is not None:
            encryption_key = worker_obj['workerTypeData']['encryptionKey']
            verify_key = worker_obj['workerTypeData']['verificationKey']
        ...
        if not ('proofData' in worker_obj['workerTypeData'] and \
                worker_obj['workerTypeData']['proofData']):
